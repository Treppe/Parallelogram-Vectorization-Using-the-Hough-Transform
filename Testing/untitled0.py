...
def hough_transform(points, theta_res=1, rho_res=1):
    ...
    '''
    x_max = np.array(np.ceil(np.amax(points[:, 0])), dtype = int)
    y_max = np.array(np.ceil(np.amax(points[:, 1])), dtype = int)
    ...
    for coords in points:
            thetaVal = np.gradient(coords)
            rhoVal = coords[0]*np.cos(thetaVal[0]*np.pi/180.0) - \
                    coords[1]*np.sin(thetaVal[0]*np.pi/180)
            rhoIdx = np.nonzero(np.abs(rho-rhoVal) == np.min(np.abs(rho-rhoVal)))[0]
            thetaIdx = np.nonzero(np.abs(theta-thetaVal[0]) == np.min(np.abs(theta-thetaVal[0])))[0]
            H[rhoIdx[0], thetaIdx[0]] += 1
...
ho_theta_pairs, x_y_pairs = top_n_rho_theta_pairs(H1,13, rhos1, thetas1)
