...
def create_rho_theta(x_max, y_max):
    ...
    distance = np.sqrt((x_max + 10)**2 + (y_max + 10)**2)
...
def validate_perimeter(valid_sides_pairs, actual_perimeter):
    ...
    for pair in valid_sides_pairs:
        val_par_condition = (abs(actual_perimeter - pair[3]["exp_per"]) <
                             PERIMETER_T * pair[3]["exp_per"])
        delta = (abs(abs(actual_perimeter - pair[3]["exp_per"]) -
                     PERIMETER_T * pair[3]["exp_per"]))
